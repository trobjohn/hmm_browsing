...
#PATHS_LIST = './int/cleanup_list.csv'  # fallback if all_paths.csv isn't being used
...
if __name__ == '__main__':
    ...
    check_frequency = 10000  # increased to reduce overhead
    estimated_limit = 8e9    # approx 8GB per batch for safety
    
    with mp.Pool(POOL_SIZE) as pool:
        for result in pool.imap_unordered(process_user_path, user_paths, chunksize=check_frequency):
